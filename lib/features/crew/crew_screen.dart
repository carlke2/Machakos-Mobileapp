...
class _CrewScreenState extends State<CrewScreen> {
  ...
  Future<void> _handleTestPush() async {
    if (!PushService.firebaseAvailable) {
      _showSnackBar(
        'Firebase is not configured on this build (missing google-services.json).',
        isError: true,
      );
      return;
    }
    try {
      await PushService.instance.requestSelfTest();
      if (!mounted) return;
      _showSnackBar('Test push sent — check the notification shade.');
    } catch (e) {
      if (!mounted) return;
      _showSnackBar('Test push failed: $e', isError: true);
    }
  }

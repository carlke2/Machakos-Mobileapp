...
Future<void> main() async {
  ...
  // Resolve Firebase before login so AuthRepository can safely start PushService.
  final ok = await _initFirebase();
  PushService.firebaseAvailable = ok;
  ...
  if (initialHome is MainShell && ok) {
    _silentVerifySession();
    await PushService.instance
        .initialize(NotificationsApi(ApiClient.instance.dio));
  } else if (initialHome is MainShell) {
    _silentVerifySession();
  }
}
...
class _MccgEocAppState extends State<MccgEocApp> {
  ...
  void _navigateTo(PushDataPayload payload) {
    if (payload.isEmpty && !payload.shouldFocusAssignment) return;
    // All dispatch-related pushes land on Assignment and reload active task.
    // taskId/caseNumber are available on the payload for future deep screens.
    if (payload.shouldFocusAssignment || !payload.isEmpty) {
